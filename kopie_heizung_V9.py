...
# UART-Konfiguration für die serielle Kommunikation (Pin 4 für TX, Pin 5 für RX)
...
while True:
   ...
   # Heizungssteuerung
    ...
    # Senden Sie Daten an Wemos D1 Mini über die serielle Schnittstelle
    data_to_send = {
        "internal_temp": internal_temperature,
        "external_temp": external_temperature,
        "humidity": humidity,
        "program_name": target_temperature,
        "runtime": int((utime.time() - start_time) / 60) if program_running else 0
    }
    uart.write(ujson.dumps(data_to_send) + '\n')
